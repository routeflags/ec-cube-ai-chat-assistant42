...
for entry in "${MODELS[@]}"; do
  ...
  sleep 1
  # gpt-5系は推論モデルのためタイムアウトを長めに
  TIMEOUT=60
  if [[ "$model" == gpt-5* ]]; then TIMEOUT=120; fi
  ...
  RESP2=$(docker compose -f /tmp/eccube-verify/docker-compose.verify.yml exec -T eccube bash -c "curl -s --max-time $TIMEOUT http://localhost/api/ai-chat-assistant/chat -H 'Content-Type: application/json' -d '{\"message\":\"おすすめ商品を教えて\",\"session_id\":\"verify-10-tool\"}' 2>&1")
  ...
  if [ "$SIMPLE_OK" = "1" ] && [ "$TOOL_OK" = "1" ]; then
    ...
    echo "  FAIL simple=$SIMPLE_OK tool=$TOOL_OK (timeout $TIMEOUT)"

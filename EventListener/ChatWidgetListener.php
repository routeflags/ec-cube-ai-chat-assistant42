...
class ChatWidgetListener implements EventSubscriberInterface
{
    ...
    public function onDefaultFrame(TemplateEvent $event): void
    {
        ...
        $event->setParameter('chat_license_lead', $designSettings['license_lead'] ?? 'AiChatAssistant42（チャットソフトウェア）の著作権は ROUTE FLAGS Co., Ltd. に帰属し、GNU General Public License v2 (GPL-2.0-only) に基づき提供されています。');
        $event->setParameter('chat_license_item1_heading', $designSettings['license_item1_heading'] ?? '著作権');
        $event->setParameter('chat_license_item1_body', $designSettings['license_item1_body'] ?? '© 2024-2026 ROUTE FLAGS Co., Ltd. All Rights Reserved.');
        ...
        $event->setParameter('chat_license_item3_body', $designSettings['license_item3_body'] ?? '本ソフトウェアは EC-CUBE 4.2 ほか composer.json 記載のOSSを利用しています。');
        ...
        if (!empty($designSettings['terms_footer_label'])) {
            ...
            $event->setParameter('chat_terms_url', $designSettings['terms_url'] ?? 'https://blog.routeflags.com/%e5%88%a9%e7%94%a8%e8%a6%8f%e7%b4%84/');

...
- **DevOps 設定ファイルは変更しない**: `phpcs.xml.dist` / `phpstan.neon.dist` / `phpmd.xml` / `phpunit.xml.dist` / `.github/workflows/*` などの DevOps・品質ゲート設定は、指示がない限り変更しないこと。閾値の緩和で違反を隠さず、コード側をリファクタリングして解消すること。
- **静的解析の無効化コメントは禁止**: コード内に `// phpcs:ignore` / `// phpstan:ignore` / `@SuppressWarnings` / `/* phpmd:ignore */` などのコメントでリンティングや静的解析の結果を非表示にし、勝手に無効化しないこと。違反はコメントで隠さず、設計・分割・命名の改善で正攻法で解消すること。
